...
double neq_exact(double T, ThermodynamicParticle &p) {
    ...
    double eta = (p.get_spin2() % 2 == 0) ? 1.0 : -1.0;

    auto integrand = [&p, eta, T](double eng) {
        return eng * sqrt(eng * eng - p.get_mass() * p.get_mass()) / (exp(eng / T) - eta);
    };

    double integral = gauss_kronrod<double, 15>::integrate(
            integrand, p.get_mass(), std::numeric_limits<double>::infinity(), 5, 1e-9);

    return p.get_g() * integral / (2.0 * M_PI * M_PI);
}
...
double energy_density_exact(double T, ThermodynamicParticle &p) {
    ...
    auto integrand = [&p, eta, T](double eng) {
        return eng * eng * sqrt(eng * eng - p.get_mass() * p.get_mass()) / (exp(eng / T) - eta);
    };
...
double pressure_density_exact(double T, ThermodynamicParticle &p) {
    ...
    auto integrand = [&p, eta, T](double eng) {
        return pow(eng * eng - p.get_mass() * p.get_mass(), 1.5) / (exp(eng / T) - eta);
    };

    double integral = gauss_kronrod<double, 15>::integrate(
            integrand, p.get_mass(), std::numeric_limits<double>::infinity(), 5, 1e-9);

    return p.get_g() * integral / (6.0 * M_PI * M_PI);
}

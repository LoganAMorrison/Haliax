...
double qagi(Integrand f, double bound, int inf, double epsabs,
            double epsrel, double *abserr, int *neval, int *ier, size_t t_limit) {
    ...
    double alist[t_limit], blist[t_limit], elist[t_limit], rlist[t_limit];
    double rlist2[52], small = 0; /* small will be initialized in _80 */

    int id, ierro, iord[t_limit], iroff1, iroff2, iroff3, jupbnd, k, ksgn;
...
/* Test on accuracy. */
    ...
    if ((*abserr <= 100.0 * kEPMACH * defabs) && (*abserr > errbnd))
        *ier = 2;
    if (limit == 0) *ier = 1;
...
/* Initialization for main loop. */
    ...
    *abserr = kOFLOW;
    ...
    iroff3 = 0;
    ksgn = -1;
    if (dres > (1.0 - 50.0 * kEPMACH) * defabs)
        ksgn = 1;
...
double qagi(
    ...
    *abserr = 0.0;
    if (limit >= 1) {
        ...
        std::cout << "Warning: Limit < 1 in call to qagie.\n";
        return NAN;
    }
}

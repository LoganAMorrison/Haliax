...
double qext(int *n, double epstab[], double *abserr,
    ...
    *abserr = kOFLOW;
    ...
    for (i = 1; i <= newelm; i++) {
        ...
        delta2 = e2 - e1;
        err2 = fabs(delta2);
        tol2 = std::max(fabs(e2), e1abs) * kEPMACH;
        delta3 = e1 - e0;
        err3 = fabs(delta3);
        tol3 = std::max(e1abs, fabs(e0)) * kEPMACH;
        ...
        *abserr = err2 + err3;
        ...
        delta1 = e1 - e3;
        err1 = fabs(delta1);
        tol1 = std::max(e1abs, fabs(e3)) * kEPMACH;
    ...
    *abserr = fabs(result - res3la[2]) + fabs(result - res3la[1]) +
    ...
    res3la[2] = result;
    _100:
    *abserr = std::max(*abserr, 5.0 * kEPMACH * fabs(result));
    *n = NN - 1;
    return result;
}
